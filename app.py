...
def handle_expenses():
    ...
    c.execute('SELECT category, amount, date, description FROM expenses WHERE user_id = ? ORDER BY date DESC',
             (session['user_id'],))
    expenses = c.fetchall()
    conn.close()
    
    return jsonify([{'category': e[0], 'amount': e[1], 'date': e[2], 'description': e[3]} for e in expenses])
